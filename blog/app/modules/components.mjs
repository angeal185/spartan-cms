  ...
  constructor() {
    ...
    for (let i = 0; i < cnf.months.length; i++) {
      ...
      item.onclick = function(){
        ...
        ss.set('spartan_timeline', [year, month])
...
class blog_prev extends HTMLElement {
  constructor(obj, config) {
    ...
    hits = h('span'),
    hits_enabled = config.spartan_rest.hitcount.enabled,
...
class blog_item extends HTMLElement {
  constructor(obj) {
    ...
    spartan_share = h('div.spartan_share'),
    ...
    if(config.share.enabled){
      spartan_share = new share_item(config);
    }

    spartan_share.append(
...
class news_item extends HTMLElement {
  constructor(obj) {
    super(obj);
    let spartan_share = h('div.spartan_share'),
    config = ls.get('config');
    ...
    this.append(
      h('div.container.mb-3', {
        ...
        spartan_share,
        h('div.row.mt-2',
          h('div#spartan_pag.col-lg-6.mt-2'),
          h('div.col-lg-6',
            h('button.btn.btn-outline-primary.btn-sm.float-right.sh-95', {
              onclick: function(){
                window.history.back();
...
class share_item extends HTMLElement {
  constructor(config) {
    ...
    this.classList.add('spartan_share');
...
class share_block extends HTMLElement {
  constructor(config) {
    ...
    this.classList.add('spartan_share_block', s_share.side);
...
class search_box extends HTMLElement {
  constructor(obj) {
    super(obj);
    let spartan_search = ss.get('spartan_search') || '';
