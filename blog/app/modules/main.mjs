...
///
/*
        //let rurl = 'https://spartan-rest.herokuapp.com/admin/test';
        let rurl = config.spartan_rest.base_url + '/test';
          fetch(rurl, {
            method: 'GET',
...
fetch(rurl, {
  method: 'POST',
  headers: g.headers.json_cors,
  body: JSON.stringify({key: config.spartan_rest.hitcount.api, item: JSON.stringify(1578318611477)})
}).then(function(res) {
  if (res.status >= 200 && res.status < 300) {
    return res.json();
  } else {
