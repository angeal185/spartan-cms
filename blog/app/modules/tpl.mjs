...
const tpl = {
  build: function(config){
    ...
    if(config.welcome_back && ls.get('welcome_msg')){
      ...
      if(config.user_config){
        //bc.append(new user_cnf(ls.get('spartan_user_cnf')))
      }
    }
  ...
  blog: function(sort_order){
    ...
    if(config.spartan_rest.enabled && !ls.get('is_subscribed') && !ls.get('is_bot')) {
      main_sub.append(
        tpl.rest_signup(config.spartan_rest)
      )
    }
  ...
  tagcat_list: function(arr, path, dtype){
  ...
  tagcat_item: function(title, count, path, dtype){
    let ico = 'tag';
    if(dtype === 'tags'){
      ico += 's'
    }
  ...
  settings: function(){
    let config = ls.get('config'),
    current_theme = ls.get('spartan_theme'),
    ...
    if(!current_theme){
      ...
      ls.set('spartan_theme', current_theme);
    }
  ...
  unsubscribe: function(){
    ...
    config = ls.get('config').spartan_rest,
