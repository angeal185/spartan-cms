...
const utils = {
  ...
  add_hitcount: function(config, cb){
    if(!config.spartan_rest.hitcount.enabled){
      return cb(false)
    }
    ...
    idb.get({index: 'cache', id: 'hitcount'}, function(err,res){
      if(err || !res || res.date < Date.now()){
        ...
        fetch(config.spartan_rest.hitcount.db_url, {
          method: 'GET',
        ...
        .then(function(data) {
          ...
          obj.date = (Date.now() + config.spartan_rest.hitcount.max_age)
    ...
    if(!config.spartan_rest.hitcount.enabled){
      ...
      idb.get({index: 'cache', id: 'viewed'}, function(err,view_arr){
        ...
        if(view_arr.data.indexOf(item) === -1){
          ...
          let rurl = config.spartan_rest.base_url + '/hit';

          fetch(rurl, {
            method: 'POST',
            headers: g.headers.json_cors,
            body: JSON.stringify({key: config.spartan_rest.hitcount.api, item: item})
          }).then(function(res) {
            if (res.status >= 200 && res.status < 300) {
              return res.json();
            } else {
  ...
  sort_type: function(sort_order){
    ...
    sort_sel = h('select.form-control.form-control-sm', {
      onchange: function(){
        ls.set('spartan_sort', this.value);
        utils.hashchange();
      }
    }),
    op = h('option'),
