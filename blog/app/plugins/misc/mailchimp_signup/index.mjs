...
const mailchimp_signup = {
  return_input: function(i){
    ...
    return h('div.form-group',
      ...
      h('input', {
        ...
        onchange: function(){
          let val = this.value,
          spartan_subscribe = ls.get64('spartan_subscribe');
          ...
          spartan_subscribe[i.title] = val;
          ls.set64('spartan_subscribe', spartan_subscribe);
        }
      }),
      h('small.error_text')
    )
  ...
  init: function(){
    let spartan_subscribe = ls.get64('spartan_subscribe'),
    arr = [{
      title: 'email',
      type: 'email'
    },{
    ...
    if(!spartan_subscribe || spartan_subscribe === undefined){
      spartan_subscribe = {email: '', first_name: '', last_name: '', subscribed: false, date: 0};
      ls.set64('spartan_subscribe', spartan_subscribe)
    } else if(spartan_subscribe.subscribed){
