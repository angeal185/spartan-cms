...
var init = function(){
  ...
  row_main.append(
    h('div.col-3',
      h('button.btn.btn-outline-danger.btn-sm',{
        type: 'button',
        onclick: function(){
          utils.confirm('delete all news entries?', function(res){
            g.cl(res)
          })
        }
      }, 'Delete all')
  ...
  document.getElementById('main-content').append(row_main, h('div#news_view_main'), h('div#pagination'))
  ws.send({type: 'news_list'})
  window.removeEventListener('socket_ready', init,false)
}

window.addEventListener("news_list", function(evt) {
  evt = evt.detail;
